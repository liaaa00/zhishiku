...
| `frontend/src/views/chat/index.vue` | 已具备消息级引用、反馈入口、引用打开逻辑 | Vue 是满足目标体验的主实现方向 |
| `frontend/src/views/admin/index.vue` | 已具备反馈列表与处理能力 | 管理员反馈闭环主要在 Vue 管理端完成 |
...
| 优先级 | 风险 | 判定 | 最小修复建议 |
|---|---|---|---|
| P0-1 | 默认入口仍进入后端内嵌 HTML | docker-compose 只暴露 backend，README 指向 `8080/chat`/`admin` | 方案 A：增加 frontend 服务/反代并更新 README；或方案 B：同步改造内嵌页到 Vue 同等能力 |
...
- Chat Vue 视图存在 `submitFeedback`、`/api/chat/feedback`、`openSourceFile`、消息级 `sources/citations` 消费逻辑。
- Admin Vue 视图存在 `loadFeedback`、`http.get('/admin/feedback')`、`updateFeedback`、`http.put('/admin/feedback/{id}')`。
- `FRONTEND_CHAT_ACCEPTANCE.md` 已列出前端应消费的 `/api/chat`、`/api/chat/feedback`、`/api/admin/feedback`、`view_url/content_url` 等验收点。
...
后端接口字段总体已经具备支撑能力，前端 Vue 方向也已对齐；当前最大阻塞不是接口设计，而是“正式入口是否能让用户访问到这些能力”以及 `integration worktree` 未初始化导致 Reviewer 无法在集成视图评审。Leader/CI 初始化 integration 后，建议优先按第 6 节执行端到端验收。
